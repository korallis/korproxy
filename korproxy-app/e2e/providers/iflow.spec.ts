...
test.describe('iFlow Provider E2E Tests', () => {
  ...
  test.describe('OAuth Flow', () => {
    test('should open OAuth modal when clicking connect on iFlow card', async ({ page }) => {
      await page.goto('/providers')
      await page.waitForLoadState('networkidle')
      
      const iflowCard = page.locator('div').filter({ hasText: /iFlow/ }).first()
      const connectButton = iflowCard.getByRole('button', { name: 'Connect' })
  ...
  test.describe('API Request Mocking', () => {
    test('should handle mocked successful chat completion', async ({ page }) => {
      mockServer.setMockResponse('chat', ProviderResponses.iflow.success('Hello from iFlow!'))
    ...
    test('should handle mocked streaming responses', async ({ page }) => {
      const streamContent = 'This is a streaming response from iFlow'
  ...
  test.describe('Error Response Mocking', () => {
    ...
    test('should handle mocked unauthorized (401) response', async ({ page }) => {
      mockServer.setMockResponse('chat', createUnauthorizedResponse('iFlow'))
    ...
    test('should handle iFlow-specific error format', async ({ page }) => {
      mockServer.setMockResponse('chat', ProviderResponses.iflow.rateLimit())
